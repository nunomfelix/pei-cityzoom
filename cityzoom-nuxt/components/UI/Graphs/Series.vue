...
export default {
  ...
  mounted() {
    this.dc = require('dc')
    setTimeout(() => {
      this.focusChart =this.dc.seriesChart("#test");
      this.overviewChart =this.dc.seriesChart("#test-overview");
      var dimension, group;
      ...
      this.overviewChart
        ...
        .dimension(dimension)
        .group(group);
      this.overviewChart.x(d3.scaleLinear().domain([-1, this.i]));
      this.overviewChart
        .y(d3.scaleLinear().domain([this.min, this.max]))
        .seriesAccessor(function(d) {
      ...
      this.onResize({ target: { innerWidth: window.innerWidth } }, false);
      this.dc.renderAll();
